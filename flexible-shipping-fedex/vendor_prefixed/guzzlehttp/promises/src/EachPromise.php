 ...
 * @final
 */
class EachPromise implements \FedExVendor\GuzzleHttp\Promise\PromisorInterface
{
    private $pending = [];
    ...
    {
        $this->iterable = \FedExVendor\GuzzleHttp\Promise\Create::iterFor($iterable);
    ...
    {
        $this->mutex = \false;
        $this->aggregate = new \FedExVendor\GuzzleHttp\Promise\Promise(function () : void {
            ...
            while ($promise = \current($this->pending)) {
                \next($this->pending);
                $promise->wait();
                if (\FedExVendor\GuzzleHttp\Promise\Is::settled($this->aggregate)) {
                    return;
                }
            }
        });
    ...
    private function addPending() : bool
    {
        ...
        $promise = \FedExVendor\GuzzleHttp\Promise\Create::promiseFor($this->iterable->current());
