...
final class UriNormalizer
{
     ...
     */
    public static function normalize(\FedExVendor\Psr\Http\Message\UriInterface $uri, int $flags = self::PRESERVING_NORMALIZATIONS) : \FedExVendor\Psr\Http\Message\UriInterface
    {
        ...
        if ($flags & self::REMOVE_DEFAULT_PORT && $uri->getPort() !== null && \FedExVendor\GuzzleHttp\Psr7\Uri::isDefaultPort($uri)) {
            $uri = $uri->withPort(null);
        }
        if ($flags & self::REMOVE_DOT_SEGMENTS && !\FedExVendor\GuzzleHttp\Psr7\Uri::isRelativePathReference($uri)) {
            $uri = $uri->withPath(\FedExVendor\GuzzleHttp\Psr7\UriResolver::removeDotSegments($uri->getPath()));
        }
     ...
     */
    public static function isEquivalent(\FedExVendor\Psr\Http\Message\UriInterface $uri1, \FedExVendor\Psr\Http\Message\UriInterface $uri2, int $normalizations = self::PRESERVING_NORMALIZATIONS) : bool
    {
        return (string) self::normalize($uri1, $normalizations) === (string) self::normalize($uri2, $normalizations);
    }
    private static function capitalizePercentEncoding(\FedExVendor\Psr\Http\Message\UriInterface $uri) : \FedExVendor\Psr\Http\Message\UriInterface
    {
        $regex = '/(?:%[A-Fa-f0-9]{2})++/';
    ...
    private static function decodeUnreservedCharacters(\FedExVendor\Psr\Http\Message\UriInterface $uri) : \FedExVendor\Psr\Http\Message\UriInterface
    {
