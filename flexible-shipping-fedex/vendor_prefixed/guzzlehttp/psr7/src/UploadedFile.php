...
class UploadedFile implements \FedExVendor\Psr\Http\Message\UploadedFileInterface
{
    ...
    private function setStreamOrFile($streamOrFile) : void
    {
        if (\is_string($streamOrFile)) {
            ...
            $this->stream = new \FedExVendor\GuzzleHttp\Psr7\Stream($streamOrFile);
        } elseif ($streamOrFile instanceof \FedExVendor\Psr\Http\Message\StreamInterface) {
    ...
    {
        if (\false === \in_array($error, \FedExVendor\GuzzleHttp\Psr7\UploadedFile::ERRORS, \true)) {
    ...
    public function getStream() : \FedExVendor\Psr\Http\Message\StreamInterface
    {
        $this->validateActive();
        if ($this->stream instanceof \FedExVendor\Psr\Http\Message\StreamInterface) {
            return $this->stream;
        }
        /** @var string $file */
        $file = $this->file;
        return new \FedExVendor\GuzzleHttp\Psr7\LazyOpenStream($file, 'r+');
    }
    public function moveTo($targetPath) : void
    {
        ...
        if ($this->file) {
            ...
            \FedExVendor\GuzzleHttp\Psr7\Utils::copyToStream($this->getStream(), new \FedExVendor\GuzzleHttp\Psr7\LazyOpenStream($targetPath, 'w'));
            $this->moved = \true;
        }
        if (\false === $this->moved) {
