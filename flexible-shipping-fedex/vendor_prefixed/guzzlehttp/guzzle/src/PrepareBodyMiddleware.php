    ...
    public function __invoke(\FedExVendor\Psr\Http\Message\RequestInterface $request, array $options) : \FedExVendor\GuzzleHttp\Promise\PromiseInterface
    {
        ...
        if (!$request->hasHeader('Content-Type')) {
            if ($uri = $request->getBody()->getMetadata('uri')) {
                if (\is_string($uri) && ($type = \FedExVendor\GuzzleHttp\Psr7\MimeType::fromFilename($uri))) {
        ...
        return $fn(\FedExVendor\GuzzleHttp\Psr7\Utils::modifyRequest($request, $modify), $options);
    }
    /**
     * Add expect header
     */
    private function addExpectHeader(\FedExVendor\Psr\Http\Message\RequestInterface $request, array $options, array &$modify) : void
    {
