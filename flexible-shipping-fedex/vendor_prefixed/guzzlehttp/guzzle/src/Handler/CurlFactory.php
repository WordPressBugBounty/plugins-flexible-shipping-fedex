...
use FedExVendor\GuzzleHttp\Multiplexing;
...
use FedExVendor\GuzzleHttp\Psr7;
use FedExVendor\GuzzleHttp\Psr7\LazyOpenStream;
use FedExVendor\GuzzleHttp\Psr7\Uri;
use FedExVendor\GuzzleHttp\TransferStats;
use FedExVendor\GuzzleHttp\TransportSharing;
...
class CurlFactory implements CurlFactoryInterface
{
    ...
    private static function normalizeMultiplex(array $options): ?string
    {
        ...
        if (!\in_array($multiplex, [Multiplexing::EAGER, Multiplexing::WAIT, Multiplexing::REQUIRE_EAGER, Multiplexing::REQUIRE_WAIT], \true)) {
    ...
    private static function rejectRequestLevelShareWithProxyAuth(RequestInterface $request, array $options, array $conf): void
    {
        ...
        if (!self::usesProxyTunnel($request, $conf) || !self::isHttpProxyForConnectionReuse($proxy, $conf) || !self::hasAuthenticatedHttpProxyState($proxy, $conf)) {
            return;
        }
    ...
    private static function triggerConflictingCurlOptionDeprecations(array $options): void
    {
        ...
        foreach ($options['curl'] as $option => $_) {
            ...
            if ($replacement !== null) {
                \FedExVendor\trigger_deprecation('guzzlehttp/guzzle', $since, \sprintf('Passing %s in the "curl" request option is deprecated; guzzlehttp/guzzle 8.0 will reject this option because it conflicts with Guzzle-managed request handling. Use %s instead.', $name, $replacement));
                continue;
            }
            \FedExVendor\trigger_deprecation('guzzlehttp/guzzle', $since, \sprintf('Passing %s in the "curl" request option is deprecated; guzzlehttp/guzzle 8.0 will reject this option because it conflicts with Guzzle-managed cURL internals.', $name));
        }
    }
    private static function triggerUnsupportedCurlOptionDeprecations(array $options): void
    {
        ...
        foreach ($options['curl'] as $option => $_) {
            ...
            \FedExVendor\trigger_deprecation('guzzlehttp/guzzle', '7.12', \sprintf('Passing %s in the "curl" request option is deprecated; guzzlehttp/guzzle 8.0 will reject raw cURL options outside the built-in cURL handlers\' allow-list.', self::formatCurlOption($option)));
        }
    }
    private static function triggerUnsupportedRequestOptionDeprecations(array $options): void
    {
        if (\array_key_exists('stream_context', $options)) {
            \FedExVendor\trigger_deprecation('guzzlehttp/guzzle', '7.11', 'Passing the "stream_context" request option to a cURL handler is deprecated; guzzlehttp/guzzle 8.0 will reject this option because cURL handlers ignore PHP stream context options.');
        }
    }
    /**
    ...
    private static function requiresFreshConnectionForAuthenticatedProxy(RequestInterface $request, string $proxy, array $conf): bool
    {
        ...
        if (self::isSocksProxy($proxy, $conf)) {
            return !CurlVersion::supportsSocksProxyCredentialAwareConnectionReuse() && self::hasAuthenticatedSocksProxyState($proxy, $conf);
        }
