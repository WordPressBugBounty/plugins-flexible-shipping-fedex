...
use Closure;
use FedExVendor\Illuminate\Support\Reflector;
...
trait ReflectsClosures
{
    ...
    protected function firstClosureParameterTypes(\Closure $closure)
    {
        ...
        $types = collect($reflection->getParameters())->mapWithKeys(function ($parameter) {
            ...
            return [$parameter->getName() => \FedExVendor\Illuminate\Support\Reflector::getParameterClassNames($parameter)];
        })->filter()->values()->all();
        if (empty($types)) {
            throw new \RuntimeException('The given Closure has no parameters.');
        }
    ...
    protected function closureParameterTypes(\Closure $closure)
    {
        ...
        return collect($reflection->getParameters())->mapWithKeys(function ($parameter) {
            ...
            return [$parameter->getName() => \FedExVendor\Illuminate\Support\Reflector::getParameterClassName($parameter)];
        })->all();
    }
}
