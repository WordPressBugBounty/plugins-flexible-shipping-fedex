...
use FedExVendor\Illuminate\Contracts\Support\CanBeEscapedWhenCastToString;
use FedExVendor\Illuminate\Support\Traits\EnumeratesValues;
use FedExVendor\Illuminate\Support\Traits\Macroable;
use IteratorAggregate;
use stdClass;
class LazyCollection implements \FedExVendor\Illuminate\Contracts\Support\CanBeEscapedWhenCastToString, \FedExVendor\Illuminate\Support\Enumerable
{
    ...
    public function collapse()
    {
        return new static(function () {
            foreach ($this as $values) {
                if (\is_array($values) || $values instanceof \FedExVendor\Illuminate\Support\Enumerable) {
    ...
    public function flatten($depth = \INF)
    {
        $instance = new static(function () use($depth) {
            foreach ($this as $item) {
                if (!\is_array($item) && !$item instanceof \FedExVendor\Illuminate\Support\Enumerable) {
                    (yield $item);
                } elseif ($depth === 1) {
                    yield from $item;
                } else {
    ...
    public function chunkWhile(callable $callback)
    {
        return new static(function () use($callback) {
            $iterator = $this->getIterator();
            $chunk = new \FedExVendor\Illuminate\Support\Collection();
            if ($iterator->valid()) {
                $chunk[$iterator->key()] = $iterator->current();
                $iterator->next();
            }
    ...
    public function zip($items)
    {
        ...
        return new static(function () use($iterables) {
            $iterators = \FedExVendor\Illuminate\Support\Collection::make($iterables)->map(function ($iterable) {
