...
class PopupPetitionAjax implements Hookable
{
    ...
    public function handle_ajax_request()
    {
        if (empty($_POST['nonce']) || !wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['nonce'])), $this->get_ajax_action())) {
            wp_send_json_error(['message' => __('Invalid nonce.', 'flexible-shipping-fedex')], 400);
        }
        ...
        switch ($subaction) {
            ...
            case 'postpone':
                ...
                break;
            default:
                wp_send_json_error(['message' => __('Unknown action.', 'flexible-shipping-fedex')], 400);
        }
    }
    private function handle_submit_rating(): void
    {
        ...
        if ($rating <= 0 || $rating > 5) {
            wp_send_json_error(['message' => __('Invalid rating value.', 'flexible-shipping-fedex')], 400);
        }
    ...
    private function handle_submit_feedback(): void
    {
        ...
        $rating = isset($_POST['rating']) ? absint($_POST['rating']) : 0;
        if (empty($message)) {
            wp_send_json_error(['message' => __('Message is required.', 'flexible-shipping-fedex')], 400);
        }
        $subject = sprintf(__('Feedback for %s', 'flexible-shipping-fedex'), $this->plugin_slug);
        $headers = [];
        ...
        if ($sent) {
            $this->option->set_option();
            wp_send_json_success(['message' => __('Thank you for your feedback!', 'flexible-shipping-fedex')]);
        } else {
            wp_send_json_error(['message' => __('Could not send email. Please try again later.', 'flexible-shipping-fedex')], 500);
        }
    }
    /**
    ...
    private function handle_postpone(): void
    {
        ...
        wp_send_json_success(['message' => __('Postponed.', 'flexible-shipping-fedex')]);
    }
