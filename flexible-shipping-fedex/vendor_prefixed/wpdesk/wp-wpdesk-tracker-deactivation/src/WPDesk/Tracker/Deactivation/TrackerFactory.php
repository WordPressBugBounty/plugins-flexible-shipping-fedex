...
class TrackerFactory
{
     ...
     * @return Tracker
     */
    public static function createCustomTracker(\FedExVendor\WPDesk\Tracker\Deactivation\PluginData $plugin_data, $scripts = null, $thickbox = null, $ajax = null, \FedExVendor\WPDesk\Tracker\Deactivation\ReasonsFactory $reasons_factory = null)
    {
        $reasons_factory = $reasons_factory ?? new \FedExVendor\WPDesk\Tracker\Deactivation\DefaultReasonsFactory();
        if (empty($scripts)) {
            $scripts = new \FedExVendor\WPDesk\Tracker\Deactivation\Scripts($plugin_data, $reasons_factory);
        }
        if (empty($thickbox)) {
            $thickbox = new \FedExVendor\WPDesk\Tracker\Deactivation\Thickbox($plugin_data, $reasons_factory);
        }
        if (empty($ajax)) {
