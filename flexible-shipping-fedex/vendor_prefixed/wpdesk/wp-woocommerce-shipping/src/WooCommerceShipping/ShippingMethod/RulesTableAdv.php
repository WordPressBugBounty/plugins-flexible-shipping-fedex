...
namespace FedExVendor\WPDesk\WooCommerceShipping\ShippingMethod;
...
class RulesTableAdv
{
    ...
    private function get_flexible_shipping_plugin_action(): string
    {
        ...
        switch ($plugin_status) {
            case 'active':
                return sprintf(__('You can update %1$sFlexible Shipping%2$s plugin to define additional costs for this shipping method. %3$s', 'flexible-shipping-fedex'), '<a href="' . esc_url($plugin_url) . '" target="_blank">', '</a>', $this->create_update_link($slug, 'plugin-update', __('Update Now', 'flexible-shipping-fedex')));
            case 'inactive':
                return sprintf(__('You can activate %1$sFlexible Shipping%2$s plugin to define additional costs for this shipping method. %3$s', 'flexible-shipping-fedex'), '<a href="' . esc_url($plugin_url) . '" target="_blank">', '</a>', $this->create_plugins_page_link($plugin_file, __('Activate Now', 'flexible-shipping-fedex')));
            case 'not installed':
                return sprintf(__('You can install %1$sFlexible Shipping%2$s plugin to define additional costs for this shipping method. %3$s', 'flexible-shipping-fedex'), '<a href="' . esc_url($plugin_url) . '" target="_blank">', '</a>', $this->create_update_link($slug, 'plugin-install', __('Install Now', 'flexible-shipping-fedex')));
            default:
                return '';
        }
    }
    ...
    {
        if (defined('FedExVendor\FLEXIBLE_SHIPPING_VERSION')) {
            $status = 'active';
