...
namespace FedExVendor\WPDesk\View;

use FedExVendor\WPDesk\View\Renderer\SimplePhpRenderer;
use FedExVendor\WPDesk\View\Resolver\ChainResolver;
use FedExVendor\WPDesk\View\Resolver\DirResolver;
use FedExVendor\WPDesk\View\Resolver\WPThemeResolver;
/**
...
class PluginViewBuilder
{
    ...
    {
        $resolver = new \FedExVendor\WPDesk\View\Resolver\ChainResolver();
        $resolver->appendResolver(new \FedExVendor\WPDesk\View\Resolver\WPThemeResolver(\basename($this->plugin_dir)));
        foreach ($this->template_dirs as $dir) {
            $dir = \trailingslashit($this->plugin_dir) . \trailingslashit($dir);
            $resolver->appendResolver(new \FedExVendor\WPDesk\View\Resolver\DirResolver($dir));
        }
        return new \FedExVendor\WPDesk\View\Renderer\SimplePhpRenderer($resolver);
    }
    /**
