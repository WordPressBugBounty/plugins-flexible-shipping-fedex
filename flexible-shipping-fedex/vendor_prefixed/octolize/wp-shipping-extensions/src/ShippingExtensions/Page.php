...
use FedExVendor\Octolize\ShippingExtensions\Plugin\PluginFactory;
use FedExVendor\Octolize\ShippingExtensions\Plugin\PluginSorter;
use FedExVendor\Octolize\ShippingExtensions\Tracker\ViewPageTracker;
use FedExVendor\WPDesk\PluginBuilder\Plugin\Hookable;
/**
 * .
 */
class Page implements \FedExVendor\WPDesk\PluginBuilder\Plugin\Hookable
{
     ...
     */
    public function __construct(string $assets_url, \FedExVendor\Octolize\ShippingExtensions\Tracker\ViewPageTracker $view_page_tracker)
    {
    ...
    {
        \add_submenu_page(self::PARENT_SLUG, \_x('Shipping Extensions', 'Page title', 'flexible-shipping-fedex'), $this->get_menu_title(), 'manage_options', self::MENU_SLUG, [$this, 'render_page']);
    }
    /**
     * @return void
     */
    ...
    {
        \wp_enqueue_style(\FedExVendor\Octolize\ShippingExtensions\Assets::HANDLE);
    ...
    {
        $menu_title = \nl2br(\_x("Shipping\nExtensions", 'Menu Title', 'flexible-shipping-fedex'));
    ...
    {
        $plugins = \FedExVendor\Octolize\ShippingExtensions\Plugin\PluginFactory::get_plugins();
        list($plugins_priority, $categories_priority) = $this->get_plugins_and_categorites_priority();
        $sorter = new \FedExVendor\Octolize\ShippingExtensions\Plugin\PluginSorter($plugins, $plugins_priority, $categories_priority);
    ...
    private function get_plugins_and_categorites_priority() : array
    {
        ...
        $categories = \FedExVendor\Octolize\ShippingExtensions\Plugin\PluginFactory::get_categories();
        $plugins_priority = [];
        $categories_priority = [];
        if (\in_array('flexible-shipping/flexible-shipping.php', $active_plugins, \true)) {
            ...
            $categories_priority[] = $categories[\FedExVendor\Octolize\ShippingExtensions\Plugin\PluginFactory::CATEGORY_CUSTOMIZABLE_RATES];
            $categories_priority[] = $categories[\FedExVendor\Octolize\ShippingExtensions\Plugin\PluginFactory::CATEGORY_SHIPPING_LABELS];
            $categories_priority[] = $categories[\FedExVendor\Octolize\ShippingExtensions\Plugin\PluginFactory::CATEGORY_LIVE_RATES];
            $categories_priority[] = $categories[\FedExVendor\Octolize\ShippingExtensions\Plugin\PluginFactory::CATEGORY_BUNDLES];
        }
    ...
    {
        return \array_values(\FedExVendor\Octolize\ShippingExtensions\Plugin\PluginFactory::get_categories());
    }
}
