...
namespace FedExVendor\Octolize\Docs\Chat;
...
class ChatSettings
{
    ...
    private function prepare_settings(): array
    {
        ...
        if (empty($settings[self::INITIAL_MESSAGES])) {
            $settings[self::INITIAL_MESSAGES] = [__('Hi there! 👋 I’m here to help you set up and use the plugin.', 'flexible-shipping-fedex'), __('You can ask me about:
- settings and configuration,
- common errors and how to fix them,
- integrations with other tools.

Just tell me what you need help with 🙂', 'flexible-shipping-fedex')];
        }
        if (empty($settings[self::TITLE])) {
            $settings[self::TITLE] = __('Hi there! 👋', 'flexible-shipping-fedex');
        }
        if (empty($settings[self::SUBTITLE])) {
            $settings[self::SUBTITLE] = __('I’m your AI Assistant. I’ll help you quickly configure the plugin and solve any issues.', 'flexible-shipping-fedex');
        }
        if (empty($settings[self::INPUT_PLACEHOLDER])) {
            $settings[self::INPUT_PLACEHOLDER] = __('Type your question to get instant answer..', 'flexible-shipping-fedex');
        }
        if (empty($settings[self::FOOTER])) {
            $settings[self::FOOTER] = __('AI can make mistakes. Octolize has access to the conversation held in this chat.', 'flexible-shipping-fedex');
        }
        if (empty($settings[self::GET_STARTED])) {
            $settings[self::GET_STARTED] = __('New conversation', 'flexible-shipping-fedex');
        }
