...
class Shipment
{
    ...
    public function __construct(array $config = [])
    {
        ...
        $this->data = ['rateRequestType' => $config['rateRequestType'] ?? [\FedExVendor\CageA80\FedEx\Support\RateRequestType::LIST, \FedExVendor\CageA80\FedEx\Support\RateRequestType::ACCOUNT], 'pickupType' => $config['pickupType'] ?? \FedExVendor\CageA80\FedEx\Support\PickupType::DROPOFF_AT_FEDEX_LOCATION, 'requestedPackageLineItems' => [], 'customsClearanceDetail' => ['commercialInvoice' => ['shipmentPurpose' => \FedExVendor\CageA80\FedEx\Support\ShipmentPurpose::SOLD], 'dutiesPayment' => ['paymentType' => $config['paymentType'] ?? \FedExVendor\CageA80\FedEx\Support\PaymentMethod::SENDER], 'commodities' => []]];
    ...
    public function addPackage(array $package) : self
    {
        if (isset($package['weight'])) {
            ...
            if (\is_array($package['weight']) && !isset($package['weight']['units'])) {
                $package['weight']['units'] = $this->config['weightUnits'] ?? \FedExVendor\CageA80\FedEx\Support\WeightUnit::LB;
            }
        }
     ...
     */
    public function getRates(\FedExVendor\CageA80\FedEx\RateRequestControlParameters $rateRequestControlParameters = null) : \FedExVendor\CageA80\FedEx\Services\Rates\RatesResponse
    {
        $ratesService = new \FedExVendor\CageA80\FedEx\Services\Rates\RatesRequest($this->config);
    ...
    protected function validateAddress(array $address, string $type) : self
    {
        if (empty($address['postalCode'])) {
            throw new \FedExVendor\CageA80\FedEx\Exceptions\ValidationException(\ucfirst($type) . ' postal code is required');
        }
        if (empty($address['countryCode'])) {
            throw new \FedExVendor\CageA80\FedEx\Exceptions\ValidationException(\ucfirst($type) . ' country code is required');
        }
        return $this;
    }
    protected function validatePackage(array $package) : self
    {
        if (empty($package['weight']) || !\is_array($package['weight'])) {
            throw new \FedExVendor\CageA80\FedEx\Exceptions\ValidationException('Package weight is required');
        }
        if (!\in_array($package['weight']['units'], [\FedExVendor\CageA80\FedEx\Support\WeightUnit::KG, \FedExVendor\CageA80\FedEx\Support\WeightUnit::LB])) {
            throw new \FedExVendor\CageA80\FedEx\Exceptions\ValidationException('Invalid package weight units: ' . $package['weight']['units']);
        }
        if (!\is_numeric(@$package['weight']['value']) || $package['weight']['value'] < 0) {
            throw new \FedExVendor\CageA80\FedEx\Exceptions\ValidationException('Invalid package weight: ' . $package['weight']['value']);
        }
        if (($this->config['maxPackageWeight'] ?? 0) && \round($package['weight']['value'], 8) > \round($this->config['maxPackageWeight'], 8)) {
            throw new \FedExVendor\CageA80\FedEx\Exceptions\ValidationException('Package weight should be less or equal than ' . $this->config['maxPackageWeight']);
        }
        if (isset($package['dimensions'])) {
            if (empty($package['dimensions']['length'])) {
                throw new \FedExVendor\CageA80\FedEx\Exceptions\ValidationException('Package dimension length is required');
            }
            if (empty($package['dimensions']['width'])) {
                throw new \FedExVendor\CageA80\FedEx\Exceptions\ValidationException('Package dimension width is required');
            }
            if (empty($package['dimensions']['height'])) {
                throw new \FedExVendor\CageA80\FedEx\Exceptions\ValidationException('Package dimension height is required');
            }
            if (!\in_array($package['dimensions']['units'], [\FedExVendor\CageA80\FedEx\Support\DimensionUnit::CENTIMETERS, \FedExVendor\CageA80\FedEx\Support\DimensionUnit::INCHES])) {
                throw new \FedExVendor\CageA80\FedEx\Exceptions\ValidationException('Invalid package dimension units: ' . $package['dimensions']['units']);
            }
        }
        return $this;
    }
    ...
    {
        if (!\count($commodity)) {
            throw new \FedExVendor\CageA80\FedEx\Exceptions\ValidationException('Commodity could not be empty');
        }
        if (!isset($commodity['name']) || !$commodity['name']) {
            throw new \FedExVendor\CageA80\FedEx\Exceptions\ValidationException('Commodity name is required');
        }
        if (isset($commodity['customsValue'])) {
            if (empty($commodity['customsValue']['amount'])) {
                throw new \FedExVendor\CageA80\FedEx\Exceptions\ValidationException('Commodity customsValue.amount is required');
            }
            if (empty($commodity['customsValue']['currency'])) {
                throw new \FedExVendor\CageA80\FedEx\Exceptions\ValidationException('Commodity customsValue.currency is required');
            }
        }
        return $this;
    }
