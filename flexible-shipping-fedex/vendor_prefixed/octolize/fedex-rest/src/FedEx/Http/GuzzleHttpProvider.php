...
namespace FedExVendor\CageA80\FedEx\Http;

use FedExVendor\CageA80\FedEx\Contracts\HttpProvider;
use FedExVendor\CageA80\FedEx\Exceptions\FedExException;
use FedExVendor\GuzzleHttp\Client;
use FedExVendor\GuzzleHttp\Exception\ClientException;
use FedExVendor\GuzzleHttp\Exception\ServerException;
use FedExVendor\GuzzleHttp\RequestOptions;
use FedExVendor\Psr\Http\Message\MessageInterface;
class GuzzleHttpProvider implements \FedExVendor\CageA80\FedEx\Contracts\HttpProvider
{
    ...
    public function post(string $url, array $data = [], array $headers = []) : ?array
    {
        try {
            // Prepare client
            $client = new \FedExVendor\GuzzleHttp\Client($this->clientOptions());
            ...
            return $result;
        } catch (\FedExVendor\GuzzleHttp\Exception\ClientException $e) {
            ...
            throw new \FedExVendor\CageA80\FedEx\Exceptions\FedExException($error['message'] ?? $e->getMessage(), $e->getCode(), ['request' => $data, 'response' => $responseData], $e);
        } catch (\FedExVendor\GuzzleHttp\Exception\ServerException $e) {
            ...
            throw new \FedExVendor\CageA80\FedEx\Exceptions\FedExException($e->getMessage(), $e->getCode(), ['request' => $data], $e);
        }
    }
    protected function clientOptions() : array
    {
        return \array_merge([\FedExVendor\GuzzleHttp\RequestOptions::CONNECT_TIMEOUT => $this->config['timeout'] ?? 30], $this->config);
    }
    protected function parseMessageBody(\FedExVendor\Psr\Http\Message\MessageInterface $response)
    {
